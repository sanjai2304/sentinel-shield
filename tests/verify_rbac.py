...
def verify(base_url="http://127.0.0.1:8000"):
    ...
    print(f"\n=== STEP 2: ANALYST ATTEMPTS ACCESS TO ADMIN AUDIT LOGS ({base_url}/api/v1/audit-logs/) ===")
    audit_req = urllib.request.Request(f"{base_url}/api/v1/audit-logs/", headers={"Authorization": f"Bearer {token}"})
    try:
    ...
    print(f"\n=== STEP 3: ANALYST ATTEMPTS ADMIN ACTION (POST {base_url}/api/v1/resources/) ===")
    ...
    create_req = urllib.request.Request(f"{base_url}/api/v1/resources/", data=create_payload, headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"})
    try:
    ...
    print(f"\n=== STEP 4: UNAUTHENTICATED REQUEST (GET {base_url}/api/v1/resources/) ===")
    unauth_req = urllib.request.Request(f"{base_url}/api/v1/resources/")
    try:
